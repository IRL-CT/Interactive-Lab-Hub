...
Based on user feedback, we found that complex operations often caused confusion.
To address this, we added sensors so that the main functions are controlled by the developers on the computer side, while users can simply interact through sensor buttons to just pause and resume the game.
...
The controller performed smoothly during testing when we controlled it. It played music without problems and responded quickly to user inputs. The basic game flow worked as we expected. Players could start the game, hear songs, make guesses, and receive feedback from the system.
