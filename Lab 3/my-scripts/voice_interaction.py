...
MAX_TTS_CHARS = 220        # cap reply for TTS to avoid 20s speeches

# -------- stdout UTF-8 & unbuffered --------
try:
...
def tts_with_piper(text: str, out_wav: Path):
    ...
    # shorten overly long replies for faster TTS
    spoken = text.strip().replace("\n", " ")
    if len(spoken) > MAX_TTS_CHARS:
        spoken = spoken[:MAX_TTS_CHARS].rsplit(" ", 1)[0] + "…"
...
def main():
    print("=== Voice Interaction: STT → LLM → TTS ===", flush=True)
    while True:
